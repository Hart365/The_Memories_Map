...
export default function TimelinePage() {
  ...
  useEffect(() => {
    if (!selectedDay && days.length === 0 && grouped['unknown']?.length) {
      setSelectedDay('unknown')
    }
  }, [selectedDay, days, grouped])

  const dayMedia = selectedDay ? (grouped[selectedDay] ?? []) : []
  const dayMediaWithLocation = dayMedia.filter((m) => m.latitude !== null && m.longitude !== null)
            ...
            <>
              ...
              {selectedDay === 'unknown' && (
                <p className={styles.empty}>
                  These files have no capture date metadata yet, so they are grouped under "No date".
                </p>
              )}
